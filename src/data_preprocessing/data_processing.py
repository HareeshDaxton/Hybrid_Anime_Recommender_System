...
class DataPreprocessing:
    ...
    def filter_users(self, min_rating=400):
        try:
            ...
            self.rating_df = self.rating_df[self.rating_df['user_id'].isin(n_rating[n_rating>=min_rating].index)].copy()
    ...
    def split_data(self, test_size=1000 , random_state=43):
        try:
            ...
            train_indeces = self.rating_df.shape[0] - test_size
            ...
            self.x_train_arr = [x_train[:, 0], x_train[:, 1]]
            self.x_test_arr = [x_test[:, 0], x_test[:, 1]]
    ...
    def save_artifacts(self):
        try:
            artifact = {
                ...
                'amime2anime_decoded' : self.anime2anime_decoded
            }
            
            for name, data in artifact.items():
                joblib.dump(data, os.path.join(self.output_dir, f'{name}.pkl'))
                logger.info(f"{name} saved sucesfully in processed directory")
                
            joblib.dump(self.x_train_arr, X_TRAIN_ARRAY)
            joblib.dump(self.x_test_arr, X_TEST_ARRAY)
    ...
    def process_aime_data(self):
        try:
            ...
            df= df[['anime_id', 'eng_version', 'Score', 'Genres', 'Episodes', 'Premiered', 'Type', 'Members']]
